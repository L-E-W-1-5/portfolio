...
export const projects = [
    {
        ...
        details: `(Currently working on adding a demo button. Complete afterthought!). A web application with a front end made using React, Typescript and Bootstrap and a back end made with Node.js, Express and Postgres, along with an AWS S3 bucket. It acts similarly to popular movie streaming sites or with alteration to the user database plus some extra UI components, it could also be similar to YouTube.
        This has been by far the most challenging project I have created to date but also the most rewarding as I can see an immense progression in my understanding of how to plan and create larger, more complex applications. There are still many more things that need to be added, fixed, or improved but I feel like I could spend an infinite amount of time on this project and still find things to refactor. 
        I have enjoyed immensely creating this project and I'm proud of the harsh learning curve I managed to overcome and the problems I managed to solve. I also feel that equally as important (or more important) as my successes during the building of this project were the things that didnt go to plan... at all. I soon found that I had a severe lack of understanding as to how some aspects of the app would function correctly. 
        There are too many to list but as one example, I soon realised that it would be useless to have a user upload a 2gb movie file in one piece, this made loading time for playing the movie or any fast forwarding etc, intolerably long, so during some research I came across 'HLS' or 'HTTP Live Streaming', which can play video files in smaller segments each only a few seconds long, I then learnt how to use FFmpeg in order to segment the video files, modify the quality/size of the video, convert them to the right format and importantly, create an .m3u8 file which lists all the segments (addresses of where they are located in the S3 bucket) in the correct order and acts as a playlist. 
        As if this wasnt enough of an issue to overcome, imagine previously I had the settings on my S3 Bucket set to strict, I didnt want unauthorised access to my videos. I used signed URL's to allow limited access to the files without making the bucket public. This wasnt an issue when my video had been in one piece but now there were over 700, 10 second segments. I'm not completely happy with my solution to this but I created an array of all the segments from the .m3u8 file, then I created a function to loop through the array and generate a signed URL for each segment. 
        Initially I assumed that this would take far too much time, generating 700 signed URL's, 700 requests to the S3 before it would start playing the video, but it actually only took a few seconds to generate all the signed URL's, add them back into a list (the .m3u8 file) and start the video playing.
        This issue went on for longer than this with the next issue being my player on the front end not recognising the .m3u8, still having short videos that werent HLS that needed to play.. The point is I could reel off numerous other issues i had like this, I (for the most part) enjoyed the challenge and being able to persevere and I can now reflect on what I would do differently next time.. `,
        ...
        stages: [
                    ...
                    },
                    {
                        ...
                        description: "I wanted to keep the message board from taking over too much of the screen space from the movie list, so i made it expandable from the right side of the screen and collapsable back to just a small column when not in use. It is responsive to screen size so if the user has a monitor or a tablet, it will expand out just a short distance, however if the page is being viewed on mobile, it will expand to cover the width of the screen. The message board initially was permanently open but I found it not great for mobile screens especially. This was only intended for mobile users but I liked it so much that I added it for all screen sizes. Maybe in the future i will make the expanding/collapsing an animation rather than just an immediate width change, just to make it look smoother and more modern."
                    },
                    {
                        ...
                        I'd love to go back and take the time to redesign some of these components to make them look cleaner and modern but I want to focue on getting the core functionality working properly first so I have a fully working MVP to show. `
                    },
                    {
                        photo: uploadForm,
